...
auto evaluate_div_round_down(std::stack<double>& stack) -> void
{
    if (stack.size() < 2) {
        throw std::logic_error{"not enough operands for //"};
    }
...
auto evaluate_subctration_minus_one(std::stack<double>& stack) -> void
{
    if (stack.size() < 2) {
    ...
    for (auto const each : args) {
        try {
            if (each == "p") {
                ...
                evaluate_subctration_minus_one(stack);
            } else {
